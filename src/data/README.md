...
The legacy `index.html` flow still depends on the original JSON-shaped `.js` data files. Keep each modern JSON file semantically in sync with its legacy source until the legacy entrypoint is removed. The parity tests in `tests/data/parity.test.ts` enforce that split.
